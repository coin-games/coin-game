...
class AuthServiceTest {
    ...
    void 토큰_정상_재발급() {
        ...
        given(refreshTokenService.getRefreshToken("refresh:" + userId)).willReturn(refreshToken);
    ...
    void 저장된_Token_불일치시_예외() {
        ...
        given(jwtProvider.validateToken("refreshToken")).willReturn(TokenValidationResult.VALID);
        given(jwtProvider.getUserId("refreshToken")).willReturn(userId);
        given(refreshTokenService.getRefreshToken("refresh:" + userId)).willReturn("다른Token");
